...
def visualize_attention(wts,x_test_pad,word_to_id,filename):
    ...
    #wts[wts<15.] = 0 #F.softmax(wts,dim=1)
    wts = torch.clamp(wts,-10.,10.)
...
if classification_type =='binary':
    ...
    train_loader,x_test_pad,y_test,word_to_id = load_data_set(1,MAXLENGTH,model_params["vocab_size"],model_params['batch_size']) #loading imdb dataset
    ...
    binary_classfication(attention_model,train_loader=train_loader,x_test=x_test_pad,y_test=y_test,epochs=params_set["epochs"],use_regularization=params_set["use_regularization"],C=params_set["C"],clip=params_set["clip"])
    classified = True
